...
const DashBoard = () => {
    ...
    const colors = ["#f97316", "#8b5cf6", "#3b82f6", "#10b981", "#14b8a6", "#eab308"]
            ...
            <div className='max-w-7xl mx-auto px-4 py-8'>
                ...
                <div className="grid grid-cols-2 sm:flex flex-wrap gap-4 ">
                    {allResumes.map((resume, index) => {
                        ...
                        return (
                            <button key={index} className="group relative w-full sm:w-[150px] md:w-[180px] lg:w-[200px]
                              h-48 rounded-lg overflow-hidden shadow-lg flex flex-col 
                              justify-between" style={{ backgroundColor: baseColor }}>
                                <FilePenLineIcon className="size-7 group-hover:scale-105 transition-all " style={{ color: baseColor }} />
                                <p className='text-sm group-hover:scale-105 transition-all px-2 text-center' style={{ color: baseColor }}>
                                    {resume.title}
                                </p>
                                <p>
                                    Updated On {new Date(resume.updatedAt).toLocaleDataString()}
                                </p>
                ...
                {showCreateResumes && (
                    <form onSubmit={createResume} onClick={() => setShowCreateResumes(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
                        <div onCLick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-fullmax-w-sm p-6'>
                            <h2 className='text-xl font-bold mb-4'>Create a Resume</h2>
                            <input type="text" placeholder='Enter resume title' className='w-full px-4
                             py-2 mb-4 focus:border-green-600 ring-green-600' required />
                            <button className='w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors'>
                                Create Resume
                            </button>
                            <XIcon className='absolute top-4 right-4 text-slate-400 
                              hover:text-slate-600 cursor-pointer transition-colors' onClick={() => { setShowCreateResumes(false); setTitle('') }} />
                        </div>
                    </form>
                )}
                {showUploadResumes && (
                            ...
                            <div>
                                <label htmlFor="resume-input" className="block text-sm text-slate-700">
                                    ...
                                    <div className='flex flex-col items-center justify-center gap-2 border group text-slate-400
                                     border-slate-400 border-dashed rounded-md p-4 py-10 my-10 hover:border-green-500
